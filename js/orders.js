...
async function fetchOrders() {
    ...
    if (!jwt) {
        document.getElementById('orders-list').innerHTML = `<p style="color:#c00;text-align:center;">Please log in to view your orders.</p>`;
        return;
    }
...
function renderOrders(orders) {
    ...
    orders.forEach(order => {
        ...
        `;
        div.querySelector('.order-card-items').addEventListener('click', () => showOrderDetails(order.id, div));
        container.appendChild(div);
    });
}

async function showOrderDetails(orderId, cardDiv) {
    const jwt = localStorage.getItem('jwt');
